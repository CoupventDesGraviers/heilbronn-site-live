...
## Verification
Every submission is checked in exact rational arithmetic before it is merged: all triangle areas are enumerated from the decimal literals exactly, and the reported value is the true minimum for the chosen container. Directories that also include a `verify_output.json` carry the output of two independent verifiers run at submission time.
